...
`--template` オプションで、Cupola が起動する Claude Code subprocess に適用する permission テンプレートを指定します。`base` テンプレートは常に適用され、追加テンプレートで言語・ツール固有のコマンドを許可します。

| Option | 説明 | デフォルト |
|--------|------|------------|
| `--template <key>[,<key>]` | 適用する permission テンプレート | `base` のみ |
...
選択したテンプレートは `cupola.toml` の `[claude_code.permissions].templates` に保存され、手動で変更することも可能です。
...
| `[claude_code.permissions] templates` | Array of String | `[]`（= `base` のみ） | 適用する permission テンプレートキーのリスト |
...
Cupola は `--allowedTools` / `--disallowedTools` CLI フラグで Claude Code subprocess の権限を制御します。`.claude/settings.json` への書き込みは行わないため、対話型 Claude Code セッションには影響しません。

`cupola.toml` で permission を設定します：
...
利用可能なテンプレートとフィールドの詳細は[設定リファレンス](#設定リファレンス)を、セキュリティモデルの全体像は [SECURITY.md](SECURITY.md) を参照してください。
