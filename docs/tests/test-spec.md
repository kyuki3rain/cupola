...
- **T-W.2** `GithubIssueSnapshot` は enum `Open { has_ready_label, ready_label_trusted, weight } | Closed` を保持。
...
- **T-D.x.1** `close_finished && github_issue is Open` → `Idle` + `metadata.consecutive_failures_epoch = now` + `metadata.close_finished = false`。
- **T-D.x.2** `!close_finished` → 状態維持、`CloseIssue` 発行。
- **T-D.x.3** `close_finished && github_issue is Closed` → 状態維持、effects なし。
