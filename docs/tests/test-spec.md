...
- **T-P.2** `ProcessRunState` は 4 バリアント: `Running`, `Succeeded`, `Failed`, `Stale`。snake_case で round-trip。
...
- **T-W.5** `ProcessSnapshot` は `state, index, consecutive_failures: u32`。
...
- **T-E.1** `Effect` は 10 バリアントちょうど: `PostCompletedComment, PostCancelComment, PostRetryExhaustedComment, RejectUntrustedReadyIssue, PostCiFixLimitComment, SpawnInit, SwitchToImplBranch, SpawnProcess { type_, causes }, CleanupWorktree, CloseIssue`。
...
- **T-D.dr.3** `design.failed`（上限未到達）→ 状態維持、`SpawnProcess(design)`。
- **T-D.dr.4** `design.stale` → 状態維持、`SpawnProcess(design)`。
- **T-D.dr.5** `design.succeeded + design_pr.merged` → `ImplementationRunning` + `ci_fix_count` リセット。
- **T-D.dr.6** `design.succeeded + design_pr.closed` → 状態維持（PR 再作成）+ `SpawnProcess(design)` + `ci_fix_count` リセット。
- **T-D.dr.7** `design.succeeded + design_pr.open` → `DesignReviewWaiting`。
- **T-D.dr.8** プロセスレコードなし → `SpawnProcess(design)`。
...
- **T-D.df.6** `design_fix.failed`（上限未到達）→ 状態維持、`SpawnProcess(design_fix, causes)`。
- **T-D.df.7** `design_fix.stale` → 状態維持、`SpawnProcess(design_fix, causes)`。
- **T-D.df.8** `design_fix.succeeded` → `DesignReviewWaiting`。
- **T-D.df.9** プロセスレコードなし → `SpawnProcess(design_fix, causes)`。
- **T-D.df.10** `causes` は毎サイクル `WorldSnapshot` から再決定される（過去 ProcessRun の causes は参照しない）。

### 6.6 ImplementationRunning (`T-D.ir.*`)
DesignRunning と対称（`impl` / `impl_pr` / 到達先 `Completed`）。

- **T-D.ir.1–8** DesignRunning `T-D.dr.1–8` と同形。
...
DesignFixing と対称。`T-D.if.1–10` は `T-D.df.1–10` と同形。
