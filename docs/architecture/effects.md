...
| `next.state == Cancelled && snap.github_issue is Closed` | PostCancelComment | best-effort |
...
| `next.state == ImplementationReviewWaiting && (snap.pr.ci_status == failure ∥ snap.pr.has_conflict) && prev.ci_fix_count == cfg.max_ci_fix_cycles` | PostCiFixLimitComment | best-effort |
...
| 項目 | 内容 |
|------|------|
| **条件** | `prev.state ≠ next.state && next.state == Cancelled && snap.github_issue is Closed` |
| **実行方法** | 同期実行（best-effort） |
...
| 項目 | 内容 |
|------|------|
| **条件** | `next.state == ImplementationReviewWaiting && (snap.pr.ci_status == failure ∥ snap.pr.has_conflict) && prev.ci_fix_count == cfg.max_ci_fix_cycles` |
| **実行方法** | 同期実行（best-effort） |
| **処理内容** | CI/Conflict 修正上限到達コメントを投稿 |
| **メタデータ更新** | Decide が `metadata_updates.ci_fix_count = Some(prev.ci_fix_count + 1)` を設定し、Persist がその値を DB に書き込む |
...
| **ImplFix 時の追加処理** | fetch + merge `origin/{default_branch}` を実行。ローカルのマージコンフリクトは Claude Code が自力で解決する。DesignFix では実行しない（Design PR の base は impl branch であり main ではないため） |
...
cupola が GitHub Issue を**能動的にクローズする**責務（GitHub close API 呼び出し）は **CloseIssue のみ**が担当する。人手クローズは Collect が `snap.github_issue is Closed` として観測し Decide に渡すが、API 呼び出し自体は行わない。コメント投稿（遷移時・イベント時）と Issue クローズ（持続性）は独立した責務であり、両者は同一サイクル内で並立して実行される。
...
Collect:  snap.github_issue = Closed
Decide:   next.state = Cancelled（遷移あり）
          エフェクト評価: prev.state ≠ next.state && snap.github_issue is Closed → [PostCancelComment]
