...
**理由**: closed issue はいかなる状態遷移も起こさない（遷移条件に `github_issue.state == open` が含まれる）。そのため、open issue 向けの通常のトラステッド判定ルール（`TrustedAssociations::All` によるチェック省略を含む）より前に closed issue の短絡条件を適用してよい。ポーリングサイクルごとに権限チェック API を呼び出しても実質的な効果はなく、ログにノイズが生じる。
