...
stateDiagram-v2
    ...
    DesignReviewWaiting --> DesignFixing: review
    ...
    Cancelled --> Idle: close_finished && github_issue is Open
```
...
| `Cancelled` | リトライ枯渇・Issue クローズによる休止。`close_finished && github_issue is Open` で自動的に `Idle` へ遷移（Issue 再オープンがトリガー） |
...
| **`InitializeRunning`** | `github_issue is Closed` | `Cancelled` |
...
| **`DesignRunning`** | `github_issue is Closed` | `Cancelled` |
...
| **`DesignReviewWaiting`** | `github_issue is Closed` | `Cancelled` |
...
| **`DesignFixing`** | `github_issue is Closed` | `Cancelled` |
...
| **`ImplementationRunning`** | `github_issue is Closed` | `Cancelled` |
...
| **`ImplementationReviewWaiting`** | `github_issue is Closed` | `Cancelled` |
...
| **`ImplementationFixing`** | `github_issue is Closed` | `Cancelled` |
...
| **`Cancelled`** | `close_finished && github_issue is Open` | `Idle` |
...
### 遷移優先度（ImplementationReviewWaiting 状態）

1. **has_review_comments**: unresolved review thread あり → `ci_fix_count` をリセットして ImplementationFixing へ
2. **ci_status == failure**: CI check-run が failure または timed_out → `ci_fix_count` をインクリメントして ImplementationFixing へ
3. **has_conflict**: PR が mergeable でない → `ci_fix_count` をインクリメントして ImplementationFixing へ

`ci_fix_exhausted`（`ci_fix_count >= max_ci_fix_cycles`）の場合、CI/Conflict による遷移は行わない（`—`）。`has_review_comments` は上限なし。

### 遷移優先度（DesignReviewWaiting 状態）

Design PR は `.cupola/specs/` のマークダウンのみを変更するため、CI failure や conflict は遷移トリガーとしない。

1. **has_review_comments**: unresolved review thread あり → `ci_fix_count` をリセットして DesignFixing へ

## トラステッド判定

信頼判定は2つの軸の **OR** で行う。

### ロールベース（trusted_associations）

`agent:ready` ラベル付与者および PR レビューコメント投稿者の author association を確認する。
...
信頼されないユーザーがラベルを付与した場合、ラベルを削除してコメントを投稿する。`TrustedAssociations::All` 設定時はチェックをスキップ。

### ユーザー名ベース（trusted_reviewers）

`trusted_reviewers` に含まれるユーザー名（デフォルト: `["copilot-pull-request-reviewer"]`）は、author association に関係なく信頼される。bot は GitHub API 上で association が `NONE` となるため、ロールベースでは対応できない。

### レビューコメントへの適用

`has_review_comments` は、trusted なコメント（ロールベース OR ユーザー名ベースのいずれかを通過）を1件以上含む unresolved スレッドのみを対象とする。trusted なコメントを含まないスレッドは存在自体が秘匿される（遷移トリガーにならず、Claude Code にも渡されない）。

### closed issue のトラステッド判定スキップ

issue が closed 状態の場合は、`TrustedAssociations::All` を含む設定値に関わらず、`agent:ready` ラベルの有無に関わらず常に `ready_label_trusted = false` を返す。この場合は `check_label_actor` API 呼び出しも行わない。

**理由**: closed issue はいかなる状態遷移も起こさない（遷移条件に `github_issue is Open` が含まれる）。そのため、open issue 向けの通常のトラステッド判定ルール（`TrustedAssociations::All` によるチェック省略を含む）より前に closed issue の短絡条件を適用してよい。ポーリングサイクルごとに権限チェック API を呼び出しても実質的な効果はなく、ログにノイズが生じる。
