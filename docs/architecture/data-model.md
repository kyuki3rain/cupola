...
| `consecutive_failures_epoch` | `Option<DateTime<Utc>>` | `consecutive_failures` 集計の起点時刻。`Cancelled → Idle` 遷移時にセットされ、これ以降の ProcessRun のみを集計対象にする（`NULL` の場合は全期間） |
...
| `index` (DB: `idx`) | `u32` | 同 type 内の通し番号（0始まり）。※ DB スキーマではカラム名は `idx` |
| `state` | `ProcessRunState` | 実行状態 |
| `pid` | `Option<u32>` | 実行中プロセスの OS PID |
| `pr_number` | `Option<u64>` | 作成した PR 番号（design / impl のみ） |
| `causes` | `Vec<FixingProblemKind>` | 修正原因（design_fix / impl_fix のみ。常に Vec で、原因がない場合は空） |
...
| `trusted_associations` | `[Owner, Member, Collaborator]` | ロールベースの信頼判定。① `agent:ready` ラベル付与者、② PR レビュースレッドの信頼判定の両方に使用する |
| `trusted_reviewers` | `["copilot-pull-request-reviewer"]` | ユーザー名ベースの信頼判定。`trusted_associations` とは独立に、特定のユーザー（主に bot）を信頼する。レビュースレッドの trust 判定で `trusted_associations` と **OR** で評価される |
...
```
Owner > Member > Collaborator > Contributor > FirstTimer > FirstTimeContributor > None
```
...
`trusted_reviewers` に含まれるユーザー名は author association に関係なく信頼される。GitHub Copilot 等の bot は GitHub API 上で association が `NONE` となるため、ロールベースの信頼判定では対応できない。この設定で明示的にホワイトリストに入れる。

### 統合判定

コメントの trust は `trusted_associations` と `trusted_reviewers` の **OR** で判定する：
- `trusted_associations.is_trusted(author_association)` → trusted
- `trusted_reviewers.contains(author_login)` → trusted
- いずれにも該当しない → untrusted
