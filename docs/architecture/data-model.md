...
`trusted_reviewers` に含まれるユーザー名は `author_association` に関係なく信頼される。GitHub Copilot 等の bot は `authorAssociation: NONE` のため、ロールベースの信頼判定では対応できない。この設定で明示的にホワイトリストに入れる。
