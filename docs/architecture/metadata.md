...
| 操作 | タイミング | 主体 |
|------|-----------|------|
| INSERT（state=pending） | SpawnProcess エフェクト実行時（spawn 試行前） | Execute |
...
| 操作 | タイミング | 主体 |
|------|-----------|------|
| セット | プロセス起動成功時（pending → running 遷移と同時） | Execute |
...
| 操作 | タイミング | 主体 |
|------|-----------|------|
| `pending` で INSERT | SpawnProcess エフェクト実行時 | Execute |
| `running` に UPDATE | spawn 成功時（pending → running） | Execute |
