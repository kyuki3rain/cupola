...
for each issue:
  ...
  --- エフェクト評価（prev / next / snap を参照）---
  4. 遷移時エフェクト: prev.state ≠ next.state の場合に発行
  5. イベント時エフェクト: next.state + snap + prev の条件で発行
  6. 持続性エフェクト: next.state + snap の条件で発行
  --- 返却 ---
  7. (next_state, metadata_updates, effects) を返す
```
...
### エフェクトは next_state 基準で評価する

[effects.md](./effects.md) の全エフェクトは、条件式で `prev` / `next` / `snap` のどれを参照するかが明記されている。持続性エフェクト（SpawnProcess 等）は `next.state` を基準に評価されるため、遷移サイクルでも遷移先の状態で即座に発火する。

Decide は状態解決とエフェクト評価を順に行う：

1. **状態解決**: `prev` と `snap` から `next.state` + `next.metadata` を決定する
2. **エフェクト評価**: 解決済みの `prev` / `next` / `snap` を参照してエフェクトリストを構築する
   - 遷移時エフェクト: `prev.state ≠ next.state` を条件に発行
   - イベント時エフェクト: `next.state` + `snap` + `prev` の条件で発行
   - 持続性エフェクト: `next.state` + `snap` の条件で発行（遷移サイクルでも発火）

各エフェクトの条件式の詳細は [effects.md](./effects.md) を参照。
