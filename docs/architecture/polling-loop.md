...
|---------|--------|----------------|
| Running 系 + 成功、後処理も成功 | stdout パース → `git push -u origin <head_branch>` → PR 作成（GitHub API） | `state=succeeded`, `pr_number` セット, `pid` クリア, `finished_at` セット |
| Running 系 + 成功、push 失敗 / 後処理失敗 | stdout パース → push or PR 作成失敗 | `state=failed`, `error_message` セット, `pid` クリア, `finished_at` セット |

| Fixing 系 + 成功、後処理も成功 | stdout パース → `git push -u origin <head_branch>` → スレッド返信・resolve（GitHub API） | `state=succeeded`, `pid` クリア, `finished_at` セット |
| Fixing 系 + 成功、push 失敗 / 後処理失敗 | push 失敗 → スレッド返信スキップ。または返信・resolve 失敗 | `state=failed`, `error_message` セット, `pid` クリア, `finished_at` セット |
| 失敗（非 0 終了） | なし | `state=failed`, `error_message` セット, `pid` クリア, `finished_at` セット |

**push の責務分離**: Claude Code は worktree 内で local commit までを担当する（`Bash(git push*)` は base permissions の deny に含まれており Claude 側からは push できない）。Resolve は Running / Fixing いずれの後処理でも GitHub API を叩く前に Cupola 側で `worktree.push` を実行し、push に失敗した場合はスレッド返信や PR 作成をスキップして `state=failed` とする。`worktree_path` が DB に無い状態で Running / Fixing に到達した場合も同様に `state=failed` とし、error ログに issue 番号と feature_name を残す。
