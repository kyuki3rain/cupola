...
- `.cupola/specs/{feature_name}/` は削除しない。spec 資産は committed ファイルとして履歴に残す。reopen 後の SpawnInit は `spec.json` と `requirements.md` を上書き再生成し、旧 `design.md` / `tasks.md` が残留していても `/cupola:spec-design` が spec フェーズに基づいて対応する
