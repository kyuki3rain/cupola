...
trusted actor のレビュースレッドを JSON 形式で書き込む。trust 判定は `trusted_associations`（ロールベース）と `trusted_reviewers`（ユーザー名ベース）の OR で行い、いずれにも該当しないコメントは除外される。trusted なコメントを1件も含まないスレッドはファイルに書き込まれない。
