...
## Execution Steps
1. **Check Uniqueness**: Verify `.cupola/specs/` for naming conflicts (append number suffix if needed)
2. **Create Directory**: `.cupola/specs/[feature-name]/`
3. **Initialize Files Using Templates**:
   - Read `.cupola/settings/templates/specs/init.json`
   - Read `.cupola/settings/templates/specs/requirements-init.md`
...
- **Template Missing**: If template files don't exist in `.cupola/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
