...
/// application to ensure buffered logs are flushed.
pub fn init_logging(log_level: LogLevel, log_dir: Option<&Path>) -> Option<WorkerGuard> {
    let filter = EnvFilter::new(log_level.as_filter_str());

    match log_dir {
        Some(dir) => {
            let file_appender = tracing_appender::rolling::daily(dir, "cupola");
