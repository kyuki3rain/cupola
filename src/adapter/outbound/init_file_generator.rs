...
impl InitFileGenerator {
    ...
    pub fn generate_spec_directory(
        ...
        std::fs::create_dir_all(&spec_dir).with_context(|| {
            format!(
                "failed to create spec directory at {}",
                spec_dir.display()
            )
        })?;
        ...
        std::fs::write(&spec_json_path, spec_json_content)
            .with_context(|| format!("failed to write spec.json at {}", spec_json_path.display()))?;
        ...
        std::fs::write(&req_path, req_content)
            .with_context(|| format!("failed to write requirements.md at {}", req_path.display()))?;
