...
- [x] 1. GraphQL クライアントの variables 化
- [x] 1.1 (P) クエリ文字列をstaticな文字列に変更し、パラメータをvariablesに分離する
...
- [x] 1.2 (P) variables 形式での動作を検証するテストを追加する
...
- [x] 2. SessionManager での旧セッション kill 処理の追加
- [x] 2.1 (P) 同一 issue_id への二重登録時に旧プロセスを終了させる
...
- [x] 2.2 (P) 旧プロセスが kill されることをテストで検証する
...
- [x] 3. PID ファイルの TOCTOU 修正
- [x] 3.1 (P) `PidFileError` に `AlreadyExists` バリアントを追加する
...
- [x] 3.2 (P) `write_pid` を排他的ファイル作成に変更する
...
- [x] 3.3 (P) PID ファイル競合および正常作成のテストを追加する
...
- [x] 4. `bootstrap/app.rs` での `AlreadyExists` エラー処理の追加
