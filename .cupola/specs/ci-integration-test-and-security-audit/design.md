...
|-------------|---------|------------|------------|-------|
| 1.1 | 統合テストステップの実行 | Integration Tests Step | `cargo test --tests` | CI Check Job |
...
1. `actions/checkout@34e114876b0b11c390a56381ad16ebd13914f8d5 # v4`（SHA ピン留め）
2. `dtolnay/rust-toolchain@stable`（rustfmt, clippy コンポーネント付き）
3. `Swatinem/rust-cache@42dc69e1aa15d09112580998cf2ef0119e2e91ae # v2`（SHA ピン留め）
...
- ジョブ分離: `security_audit` は独自パーミッション（`contents: read`, `issues: write`, `checks: write`）が必要なため独立ジョブとする（`contents: read` は `actions/checkout` に必要な最小権限）
...
| Test Runner | cargo test `--tests` | 統合テスト実行 | 追加 |
...
- Input / validation: `tests/` 配下のすべての統合テスト（`--tests` フラグで全統合テストを対象）
...
- パーミッション `contents: read`, `issues: write`, `checks: write` を当該ジョブスコープ内のみに限定する（`contents: read` は `actions/checkout` に必要な最小権限）
...
- `security_audit` ジョブのパーミッションは `contents: read`, `issues: write`, `checks: write` に限定し、最小権限の原則を遵守する（`contents: read` は `actions/checkout` に必要）
