...
- [x] 5. ログ出力の検証テストを追加する
  - (Done in this PR) 状態遷移のリグレッションテスト: `TransitionUseCase::apply()` の状態遷移フローが壊れていないことを確認する（ログ内容のアサーションは行わない）
  - (TBD) 状態遷移ログのユニットテスト: `TransitionUseCase::apply()` 呼び出し後に期待するフィールド（from, to, issue_number）が INFO ログに含まれることを検証する
  - (TBD) PR 作成・fixing 完了・プロセス終了ログの統合テスト: 各フローを実行し、期待するログメッセージとフィールドが出力されることを検証する
