...
- [x] 1. GitHubApiError enum を定義する
- [x] 1.1 GitHub API の HTTP エラーを型安全に表現できるエラー型を作成する
...
- [x] 1.2 HTTP ステータス等からエラー種別を分類できる変換ロジックを実装する
...
- [x] 2. `github_rest_client.rs` のエラーサイトを置き換える
- [x] 2.1 5 箇所のエラーサイトを `classify_http_error` に置き換える
...
- [x] 3. `github_client_impl.rs` にリトライロジックを追加する
- [x] 3.1 汎用リトライヘルパー `with_retry` を実装する
...
- [x] 3.3 `GitHubClientImpl` の各メソッドで `with_retry` を使用する
...
- [x] 4. ユニットテストを追加する
- [x] 4.1 (P) `classify_http_error` のユニットテストを追加する
...
- [x] 4.2 (P) `with_retry` のユニットテストを追加する
...
- [x] 5. 既存テストとの互換性を確認する
- [x] 5.1 既存のテストが変更なしでコンパイル・パスすることを確認する
