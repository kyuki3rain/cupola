...
- **Key Findings**:
  ...
  - `reqwest::Response` は body 消費後に headers にアクセス不可のため、headers を先に取り出す必要がある
  - GitHub Timeline API はタイムラインを**新しい順**で返すため、既存の `.rev()` ロジックとの整合が必要
...
- **Context**: `resp.json().await` を呼ぶと body が消費され、以降 `resp.headers()` は使えなくなる（もしくは空になる）
- **Findings**:
  - `reqwest::Response` の `headers()` メソッドは body 消費前後でも参照可能だが、`json()` / `text()` / `bytes()` 消費後は所有権が移動するため直接アクセス不可
...
- **Implications**: 専用クレート不要。ユーティリティ関数 `parse_link_header(header_value: &str) -> Option<String>` で実装する
...
- **Findings**:
  - タイムラインは**新しい順（降順）**で返される
  - 最初のページに最新イベントが含まれるため、`agent:ready` が最近付与された場合は早期終了が可能（ただし最悪ケースでは全ページ取得が必要）
  - 最大ページ数（10ページ = 最大 1000 イベント）は実用的な上限として十分
- **Implications**: 全イベントを収集してから `.rev()` で逆順走査する既存ロジックを維持する
...
| ページごとに検索して早期終了 | 各ページで検索し見つかったら即返却 | メモリ効率が良い | タイムラインが降順なので後ページほど古いイベント → 早期発見の恩恵が限定的 | 不採用（実装複雑化に対し恩恵少） |
