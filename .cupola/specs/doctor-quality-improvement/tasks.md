...
- [x] 1. ConfigLoader port の定義と bootstrap 実装

- [x] 1.1 application 層に ConfigLoader trait と関連型を定義する
...
- [x] 1.2 bootstrap 層に TomlConfigLoader を実装する
  - `TomlConfigLoader` 構造体を `bootstrap/toml_config_loader.rs` に定義する
...
- [x] 2. DoctorUseCase の診断ロジックを実装する

- [x] 2.1 DoctorUseCase の骨格と結果型を定義し、check_toml を実装する
...
- [x] 2.2 (P) check_git を実装する（環境依存対応）
...
- [x] 2.3 (P) check_gh を実装する（未インストール vs 認証失敗の区別）
...
- [x] 2.4 (P) check_gh_label を実装する（agent:ready の JSON パース）
...
- [x] 2.5 (P) check_steering を実装する（ファイル限定カウント）
  ...
  - カウントが 0 の場合は `CheckStatus::Fail` を返す
  - `read_dir().next().is_some()` による不正確な確認は使用しない
  - _Requirements: 6.1, 6.2, 6.3_

- [x] 2.6 (P) check_db を実装する
...
- [x] 3. CLI サブコマンド統合と表示ロジックの実装

- [x] 3.1 cli.rs に Doctor サブコマンドを追加する
...
- [x] 3.2 bootstrap/app.rs に Doctor ハンドラを統合する（Err 返却・表示ロジック）
...
- [x] 4.1 (P) toml チェックのユニットテストを追加する（tempdir ベース）
  - `tempfile::TempDir` を使って MockConfigLoader で `check_toml` が `Ok` を返すことを検証する
  - toml ファイルが存在しない場合に `Fail` を返すことを検証する
  - 必須フィールド（`default_branch`）が欠けた場合に `Fail` を返すことを検証する
  - _Requirements: 2.1, 2.2, 2.3_

- [x] 4.2 (P) steering チェックのユニットテストを追加する（tempdir ベース）
...
- [x] 4.5 (P) agent:ready ラベルの JSON パーステストを追加する
  - `[{"name":"agent:ready"}]` を入力として `parse_label_json` が `true` を返すことを検証する
  - `[{"name":"not-agent:ready"}]` で誤検知なしに `false` を返すことを検証する
  - 不正 JSON（空文字列等）で `Err` を返すことを検証する
  - _Requirements: 5.1, 5.2, 5.3_

- [x] 4.6 (P) DoctorUseCase の統合テストをモック ConfigLoader で追加する
  ...
  - 全チェックが `Ok` のシナリオと、toml が `Fail` のシナリオを検証する
  - _Requirements: 4.4_
