...
- [x] 1. AuthorAssociation と TrustedAssociations のドメイン型を実装する
...
- [x] 2. trusted_associations 設定フィールドを Config と TOML ローダーに追加する

- [x] 2.1 (P) Config 構造体と TOML 読み込みロジックを拡張する
...
- [x] 2.2 設定値の検証ロジックを追加する
  - `Config::validate()` に `trusted_associations` の検証を追加する
  - 不正な association 文字列が含まれる場合は `ConfigError` を返す
  - 起動時エラーとしてログ出力し処理を停止させる
  - _Requirements: 1.5_

- [x] 3. GitHubClient ポートと ReviewComment 構造体を拡張する

- [x] 3.1 (P) Timeline API・Permission API・ラベル操作メソッドをポートに定義する
...
- [x] 3.2 (P) ReviewComment に author_association フィールドを追加する
...
- [x] 4. REST クライアントに Timeline API と Permission API を実装する
  - タスク 3.1 の完了が前提（ポートインターフェースの依存）

- [x] 4.1 Timeline API でラベル付与者のログインを取得する機能を実装する
...
- [x] 4.2 Permission API でユーザーの association を取得する機能を実装する
...
- [x] 5. (P) GraphQL クライアントに authorAssociation フィールドの取得を追加する
...
- [x] 6. association_guard モジュールを実装する
  - タスク 3.1 および タスク 4 の完了が前提

- [x] 6.1 ラベル付与者の association チェックロジックを実装する
...
- [x] 7. ポーリング処理に association チェックを組み込む
...
- [x] 8. (P) PR レビューコメントのフィルタリングを実装する
...
- [x] 9. セキュリティドキュメントと設定例を整備する

- [x] 9.1 (P) SECURITY.md を作成する
