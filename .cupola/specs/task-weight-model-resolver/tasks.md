...
- [ ] 1. ドメイン型（TaskWeight / Phase / ModelConfig）の実装

- [ ] 1.1 (P) TaskWeight enum を実装する
...
- [ ] 1.2 (P) Phase enum と State からの変換を実装する
...
- [ ] 1.3 ModelConfig と resolve ロジックを実装する
...
- [ ] 1.4 Issue エンティティの model フィールドを weight に置き換える
  - `Issue.model: Option<String>` を削除し `weight: TaskWeight` を追加する（1.1 が完了していること）
  - `weight` のデフォルト値は `TaskWeight::Medium` とする
  - _Requirements: 3.1, 3.2, 3.3_

- [ ] 2. Config の ModelConfig 埋め込みと TOML パース拡張

- [ ] 2.1 Config に models フィールドを追加する
...
- [ ] 2.2 CupolaToml に TOML パース用型を追加して Config に変換する
...
- [ ] 4. ポーリングの label 検出とモデル解決の変更

- [ ] 4.1 Step1 での weight ラベル検出を実装する
...
- [ ] 4.2 Step7 のモデル解決を config.models.resolve() 経由に変更する
...
- [ ] 5. init テンプレートと doctor コマンドの更新

- [ ] 5.1 (P) cupola.toml テンプレートに models セクションの凡例を追加する
  - `InitFileGenerator` の `CUPOLA_TOML_TEMPLATE` に `[models]` セクションのコメント付き記法例（weight 別・phase 別）を追加する
  - _Requirements: 7.1_

- [ ] 5.2 (P) DoctorUseCase の weight ラベルチェックを実装する
...
- [ ] 6.2 (P) Config TOML パースと変換の統合テストを実装する
...
- [ ] 6.3 (P) SQLite の weight 読み書き統合テストを実装する
