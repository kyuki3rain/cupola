...
#### 受け入れ基準
1. When foreground モードで `cupola start` が実行され、PID ファイルが存在し記載 PID のプロセスが生存中である, the Cupola shall 起動を中断し、`already running` と `pid=<PID>` を含むエラーを返す。
...
4. If PID ファイルの読み込みで IO エラー、または PID ファイル内容の不正による parse / range error が発生した, the Cupola shall 起動を中断し当該エラーを返す。
...
3. While 同一 `config_dir` を共有する Cupola プロセスが1つ動作中（foreground または daemon）, the Cupola shall 同一 PID ファイルパス（`<config_dir>/cupola.pid`）を共有することで相互排他を保証する。
