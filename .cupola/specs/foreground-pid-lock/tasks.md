...
- [x] 1. 共有 PID チェックヘルパーの実装とリファクタリング

- [x] 1.1 `start_daemon` の PID チェックロジックを共有ヘルパー関数として抽出する
...
- [x] 1.2 `check_and_clean_pid_file` のユニットテストを実装する
...
- [x] 2. foreground モードへの PID ファイル保護の追加

- [x] 2.1 `start_foreground` に PID ファイルチェック・書き込み・クリーンアップを追加する
...
- [x] 2.2 `start_foreground` の PID 保護動作を検証するテストを実装する
