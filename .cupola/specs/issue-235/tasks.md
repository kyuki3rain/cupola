...
- [x] 3. ImplementationFixing 専用プロンプト生成を実装する
  - 実装コード修正に特化した文言を含むプロンプトを生成する機能を追加する（現行と同等の内容）
  - コミット指示は `fix:` プレフィックスを用いる（現状維持）
  - タスク 1 で抽出したヘルパーを呼び出してセクションを組み立てる
  - _Requirements: 2.1, 2.2, 2.3, 2.4, 2.5, 2.6, 2.7_

- [x] 4. セッション設定の分岐先を fixing 種別ごとのプロンプト生成に切り替える
...
- [x] 5.2 (P) `ImplementationFixing` 専用テストを追加する
...
- [x] 5.3 (P) 既存の `fixing_prompt_*` テスト群を `ImplementationFixing` 状態でも実行されるよう拡張する
...
- [x] 5.4 (P) `DesignFixing` の causes ハンドリングテストを追加する
