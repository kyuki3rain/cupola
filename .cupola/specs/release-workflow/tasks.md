...
- [x] 1. Release ワークフローの基本構造とトリガー設定
  ...
  - GitHub Actions の `on.push.tags` は glob として解釈されることに留意し、タグパターン `v[0-9]*.[0-9]*.[0-9]*` でセマンティックバージョンらしきタグ push イベントを大まかにフィルタする（サフィックス付きのタグも含まれ得る）
  - 上記の glob でフィルタされたイベントに対して、ワークフロー内のジョブ開始時に `GITHUB_REF_NAME` などから取得したタグ名が正規表現 `^v[0-9]+\.[0-9]+\.[0-9]+$` にマッチするか厳密に検証し、マッチしない場合はジョブを即座に失敗させるステップを追加する
