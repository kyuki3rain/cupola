...
**Impact**: 5 つの既存ドキュメントファイルに対して最小限の文字列変更のみを加える。コード・ロジック・構造への変更はない。なお、本 PR には仕様管理ファイル（`.cupola/specs/add-cc-sdd-links/` 配下の `spec.json`, `requirements.md`, `design.md`, `tasks.md`, `research.md`）も含まれるが、これらはドキュメント変更を管理するための補助ファイルであり、本番コードへの影響はない。
...
> **スコープ補足**: 仕様管理ファイル（`.cupola/specs/add-cc-sdd-links/` 配下）はこの PR のスコープに含まれる。これらは Cupola のスペック駆動開発プロセスの一部として生成された補助ファイルであり、プロダクションコードとは独立している。
