...
- [ ] 1. `dump_schema()` テスト用ヘルパーを追加する
- [ ] 1.1 `SqliteConnection` に `#[cfg(test)]` 付きの `dump_schema()` メソッドを追加する
...
- [ ] 2. マイグレーションテスト基盤を整備する
- [ ] 2.1 `tests/migrations/` ディレクトリ構造を作成する
  - `tests/migrations/fixtures/` ディレクトリを作成する
  - `tests/migrations/snapshots/` ディレクトリを作成する
  - _Requirements: 2.1_

- [ ] 2.2 `v0001-initial.db` fixture を生成して commit する
...
- [ ] 3. `tests/migrations/mod.rs` にテストを実装する
- [ ] 3.1 `load_fixture()` ヘルパーを実装する
  - `(SqliteConnection, tempfile::TempDir)` を返す関数として実装し、`TempDir` によって一時ファイルの生存期間を管理する
  - _Requirements: 3.1, 4.1_

- [ ] 3.2 `normalize_schema()` ヘルパーを実装する
  - CRLF→LF・末尾空白除去・空行除去を行う関数として実装する
  - _Requirements: 4.3_

- [ ] 3.3 `migrate_v0001_is_idempotent` テストを実装する
