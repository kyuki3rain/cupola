...
- [x] 1. `graceful_shutdown` のループ終了判定を修正する
- [x] 1.1 ループ継続条件を `session_mgr.count() != 0` に変更する
...
- [x] 2. `is_process_alive` 自由関数を実装し `recover_on_startup` を拡張する
- [x] 2.1 Unix シグナル 0 によるプロセス生存確認関数を実装する
...
- [x] 2.2 `recover_on_startup` に孤児プロセス kill 処理を追加する
...
- [x] 3. テストを追加してすべての要件を検証する
- [x] 3.1 `graceful_shutdown` のループ判定テストを追加する
...
- [x] 3.2 (P) `is_process_alive` のユニットテストを追加する
...
- [x] 3.3 (P) `recover_on_startup` での生存プロセス kill テストを追加する
  - `current_pid = Some(pid)` かつプロセスが生存している場合に SIGKILL が呼ばれ、`current_pid` が `None` にクリアされることを検証するテストを追加する
  - _Requirements: 2.1, 2.2_

- [x] 3.4 (P) `recover_on_startup` での死亡済みプロセスクリアテストを追加する
