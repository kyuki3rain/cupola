...
- [ ] 1. CLIフラグの追加
- [ ] 1.1 `Command::Init` に `--upgrade` フラグを追加する
...
- [ ] 2. `FileGenerator` ポートのシグネチャ変更
- [ ] 2.1 `FileGenerator` トレイトのメソッドシグネチャに `upgrade: bool` を追加する
...
- [ ] 3. `InitUseCase` へのフラグスレッド
- [ ] 3.1 `InitUseCase` に `upgrade: bool` フィールドを追加し、`FileGenerator` メソッドに渡す
...
- [ ] 5. Bootstrap の接続と結果表示の更新
- [ ] 5.1 `app.rs` の Init ハンドラを `upgrade` フラグに対応させる
