...
5. The cupola shall CI失敗検知のために新たに追加するAPIコール（PRのhead SHA取得およびGitHub Checks API呼び出し）をreview_waiting Issue あたり最大2回/サイクルに収める（既存のmerge判定のための `GET /repos/{owner}/{repo}/pulls/{number}` 呼び出しはこの上限に含めない）
