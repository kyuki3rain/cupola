...
- [x] 1. FixingProblemKind ドメイン型と Issue エンティティの拡張
- [x] 1.1 FixingProblemKind 列挙型を domain 層に追加する
...
- [x] 1.2 Issue エンティティに fixing_causes フィールドを追加してDBスキーマを移行する
...
- [x] 2. GitHub CI確認・conflict確認 API の追加
- [x] 2.1 GitHubClient ポートに CI check-runs と PR mergeable 取得メソッドを追加する
...
- [x] 2.2 (P) CI check-runs 取得の REST 実装を OctocrabRestClient に追加する
...
- [x] 2.3 (P) PR mergeable フィールド取得の REST 実装を OctocrabRestClient に追加する
...
- [x] 2.4 GitHubClientImpl ファサードに新規メソッドの委譲を追加する
  - `GitHubClientImpl` の `GitHubClient` impl ブロックに `get_ci_check_runs` と `get_pr_mergeable` を追加する
  - 両メソッドとも `rest` フィールドに委譲する
  - _Requirements: 1.1, 2.1_

- [x] 3. CI エラー・conflict 情報の入力ファイル書き出し機能を追加する
- [x] 3.1 (P) CI エラーログを ci_errors.txt に書き出す機能を実装する
...
- [x] 3.2 (P) conflict 情報を conflict_info.txt に書き出す機能を実装する
...
- [x] 4. fixing プロンプトを問題種別リストに基づく動的生成へ変更する
...
- [x] 5. step4 PR 監視ロジックを拡張して CI 失敗・conflict を検知する
- [x] 5.1 step4 に CI 失敗検知を追加する
...
- [x] 5.2 step4 に conflict 検知を追加する
...
- [x] 5.3 複数問題の同時収集と優先順位制御・イベント発行を実装する
...
- [x] 6. prepare_inputs を fixing_causes に基づく入力ファイル準備方式へ更新する
...
- [x] 7. 統合テストとユニットテストの追加
- [x] 7.1 build_fixing_prompt の各パターンのユニットテストを追加する
...
- [x] 7.2 step4 の CI 失敗・conflict 検知の統合テストを追加する
...
- [x] 7.3 prepare_inputs の入力ファイル書き出しを検証するテストを追加する
