...
- [x] 1. Config と設定読み込みに同時実行数上限を追加
- [x] 1.1 (P) domain 層の Config に同時実行数上限フィールドを追加する
  - Config 値オブジェクトに同時実行セッション数の上限を保持する Optional フィールドを追加する
  - 未設定（None）は制限なしを意味する。0 以下の値はバリデーションエラーとする
  - _Requirements: 1.1, 1.2, 1.3_

- [x] 1.2 (P) bootstrap 層の設定ローダーで TOML からの読み込みとマッピングを実装する
...
- [x] 2. SessionManager にセッション数カウント機能を追加
- [x] 2.1 (P) SessionManager に実行中セッション数を返すメソッドを追加する
...
- [x] 3. PollingUseCase の Step 7 に同時実行数の上限チェックを追加
- [x] 3.1 step7_spawn_processes のプロセス起動ループ冒頭に上限チェックを追加する
  - ループ内で毎回 SessionManager の実行中セッション数と Config の上限値を比較する
  - 上限以上の場合はログを出力してループを抜ける（残りの Issue は次の polling サイクルで再試行）
  - 上限未設定（None）の場合はチェックをスキップし、全 Issue に対してプロセスを起動する
...
- [x] 4. status コマンドに実行中プロセス数と上限の表示を追加
- [x] 4.1 status コマンドの出力に実行中プロセス数のサマリー行を追加する
...
- [x] 4.2 PID の永続化フローを実装する
  - step7 で spawn 成功時に `Issue.current_pid = Some(pid)` を DB に保存する（state 更新と同一トランザクション推奨）
  - `collect_exited()` でプロセス終了を検知した際に `current_pid = None` にクリアし、state を更新して DB に保存する
  - _Requirements: 5.1_

- [x] 5. 統合テストで同時実行数制限の動作を検証
- [x] 5.1 上限設定時にプロセス起動数が制限されることを検証するテストを作成する
  - 上限を 2 に設定し 3 つの needs_process な Issue がある場合、2 つだけ起動されることを検証する
  - 上限未設定時に全 Issue が起動されることを検証する
  - 上限を 0 に設定した場合にバリデーションエラーとなることを検証する
  - _Requirements: 2.1, 2.2, 2.3, 3.1_

- [x] 5.2 (P) プロセス終了後の再試行動作を検証するテストを作成する
