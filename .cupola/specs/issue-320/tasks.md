...
- [ ] 1. ドメインモデルと DB スキーマの拡張
- [ ] 1.1 (P) `Issue` エンティティと `MetadataUpdates` に `body_hash` フィールドを追加する
...
- [ ] 1.3 (P) `persist_decision` の「変更なし」早期リターン条件を更新する
...
- [ ] 2. SHA-256 ユーティリティと `BodyTamperedError` の追加
- [ ] 2.1 (P) `sha2` クレートを追加し `sha256_hex` ユーティリティ関数を実装する
...
- [ ] 2.2 (P) `BodyTamperedError` エラー型を定義する
...
- [ ] 3. SpawnInit 時のハッシュ計算・保存
- [ ] 3.1 `prepare_init_handle` をハッシュを計算して返すよう変更する
...
- [ ] 3.2 `spawn_init_task` でハッシュを保存してインメモリ `Issue` を更新する
...
- [ ] 4. SpawnProcess 前のハッシュ検証と改変応答
- [ ] 4.1 `prepare_process_spawn` にハッシュ比較ロジックを追加する
...
- [ ] 4.2 `execute_one` の `Effect::SpawnProcess` ブランチに改変応答を実装する
