...
- [x] 1. 未使用コードの削除
- [x] 1.1 `CupolaError`（`application/error.rs`）を削除し、参照を全て除去する
...
- [x] 1.2 `domain/check_result.rs`（デッドコード）を削除し、参照を全て除去する
  - `src/domain/check_result.rs` ファイルを削除する
  - `mod.rs` 等から `check_result` モジュール宣言を削除する
  - 削除後に `cargo build` が警告なしで成功することを確認する
  - _Requirements: 1.2, 1.3, 1.4_

- [x] 2. `SignalPort` の分離と `NixSignalSender` の adapter 層への移動
- [x] 2.1 (P) `SignalPort` トレイトを `application/port/signal.rs` に移動する
...
- [x] 2.2 (P) `NixSignalSender` を `adapter/outbound/nix_signal_sender.rs` に移動する
...
- [x] 2.3 `StopUseCase` の import パスを修正し、bootstrap の DI を `with_signal_sender` に変更する
...
- [x] 3. (P) `DoctorUseCase` への `CommandRunner` ポート注入
- [x] 3.1 `DoctorUseCase` 構造体に `CommandRunner` フィールドを追加し、コマンド呼び出しをポート経由にする
...
- [x] 3.2 bootstrap での注入を追加し、既存テストを `MockCommandRunner` 経由に更新する
...
- [x] 4. `InitUseCase` のポート依存化
- [x] 4.1 (P) `DbInitializer` ポートを定義し、`SqliteConnection` に実装させる
...
- [x] 4.2 (P) `FileGenerator` ポートを定義し、`InitFileGenerator` に実装させる
...
- [x] 4.3 `InitUseCase` のシグネチャを変更し、bootstrap DI とテストを更新する
...
- [x] 5. エラー型統一の確認と全体ビルド・テスト検証
- [x] 5.1 use case 戻り値型の `anyhow::Result` 統一を確認し、不整合を修正する
...
- [x] 5.2 `cargo build`、`cargo test`、`cargo clippy --all-targets -- -D warnings` を全パスさせる
