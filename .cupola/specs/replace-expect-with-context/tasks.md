...
- [x] 1. (P) prompt.rs の build_session_config をエラー伝播に対応させる
- [x] 1.1 (P) build_session_config の戻り値型を Result に変更し、pr_number 欠如時にエラーを返す
...
- [x] 2. step7_spawn_processes でのエラースキップ処理を追加する
- [x] 2.1 step7_spawn_processes で build_session_config のエラー時に該当 Issue をスキップする
...
- [x] 3. (P) transition_use_case.rs の expect() をエラー伝播に変更する
- [x] 3.1 (P) reset_for_restart 後の find_by_id で None の場合にエラーを伝播させる
...
- [x] 4. (P) clippy lint によるリグレッション防止を設定する
- [x] 4.1 (P) Cargo.toml の clippy lint 設定に expect_used = "deny" を追加する
  - 既存の `[lints.clippy]` セクションに `expect_used = "deny"` を追記する
  - `all = "warn"` はすでに設定済みのため変更不要
  - _Requirements: 3.1, 3.2_

- [x] 4.2 (P) src/lib.rs でテストコードの expect_used lint を許可する
  - `#![cfg_attr(test, allow(clippy::expect_used))]` をクレートレベルアトリビュートとして `src/lib.rs` 先頭付近に追記する
  - `#[cfg(test)]` ブロック内の `expect()` がエラーにならないことが目的
  - _Requirements: 3.3, 3.6_

- [x] 4.3 (P) tests/ 配下の integration test ファイルで expect_used lint を許可する
...
- [x] 5. テスト・lint 検証を実施する
- [x] 5.1 build_session_config の各状態に対するユニットテストを追加する
